...
/**
 * Fetch a JSON file from the given location.
 *
 * @param location - The Snap location.
 * @param path - The path of the JSON file to fetch.
...
async function fetchJson<Type>(
  ...
  const json = await location.fetch(path);
  json.result = JSON.parse(json.toString());
  return json as VirtualFile<Type>;
}
...
export async function fetchSnap(snapId: string): Promise<SnapFiles> {
  ...
  const files: SnapFiles = {
    manifest,
    packageJson: await fetchJson<NpmSnapPackageJson>(location, 'package.json'),
  ...
  return manifest.source.locales
    ? await Promise.all(
        manifest.source.locales.map(async (filePath) =>
          fetchJson<LocalizationFile>(location, filePath),
        ),
      )
    : [];
}
